...
#eval IO.println s!"The result of squaring the integer 4573 and adding 3 is {result1}"
...
#eval IO.println s!"The result of applying the 2nd sample function to (7 + 4) is {result2}"
...
def sampleFunction3 (x : Int) :=
  if x > 100 then
    ...
    2*x*x + x - 37
#eval IO.println s!"The result of applying sampleFunction3 to 2 is {sampleFunction3 2}"
...
-- You can prove theorems about your functions. The following theorem states that
-- for any natural number a, adding 2 twice produces a value equal to a + 4.
...
-- (· + 2) is syntax sugar for (fun x => x + 2). The parentheses + · notation is useful
-- for defining simple anonymous functions.
#eval twice (· + 2) 10

-- Enumerated types are a special case of inductive types in Lean, which we will learn about later.
...
-- Weekday has 7 constructors/elements. The constructors live in the Weekday namespace.
-- Think of sunday, monday, …, saturday as being distinct elements of Weekday,
-- with no other distinguishing properties. The command #check prints the type of a term in Lean.
...
-- The method toString converts values of any type that implements the class ToString.
...
-- Matching on a parameter like in the previous definition is so common that Lean provides
-- syntax sugar for it. The following function uses it.
