...
BATTERY_CAPACITY_WH = 1_000_000  # 1 MWh
BATTERY_ENERGY_PER_INTERVAL = 50_000  # 50 kWh (5% of capacity)
BATTERY_POWER_W = 600_000 # 600 kW, derived from 50kWh per 5-min interval
...
def pre_calculate_optimal_thresholds(profitability_data):
    ...
    # Buy when energy is in the cheapest 15% of the week
    buy_price_threshold = np.percentile(all_energy_prices, 15)
    # Sell when energy is in the most expensive 15% of the week (top 85%)
    sell_price_threshold = np.percentile(all_energy_prices, 85)
    ...
    print(f"Buy energy when price < ${buy_price_threshold / 1000:.4f}/kWh (15th percentile +/- {THRESHOLD_BUFFER_PERCENT*100}%)")
    print(f"Sell energy when price > ${sell_price_threshold / 1000:.4f}/kWh (85th percentile +/- {THRESHOLD_BUFFER_PERCENT*100}%)")
    print("-----------------------------------------\n")
...
def run_weekly_arbitrage_simulation():
    ...
    output_file = "arbitrage_with_mid_battery_result.json"
